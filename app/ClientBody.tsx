...
type RiskGovernorInput = {
  startBalance: number;
  equity: number;
  sessionLossSoFar: number;
  todayLossSoFar: number;
...
const LS = {
  armed: 'ust-checklist-armed',
  thresholdPct: 'ust-checklist-thresholdPct',
  doNotRiskCapital: 'ust-checklist-doNotRiskCapital',
  sessionMaxLossMode: 'ust-checklist-sessionMaxLossMode',
  sessionMaxLossValue: 'ust-checklist-sessionMaxLossValue',
  dailyMaxLossPct: 'ust-checklist-dailyMaxLossPct',
  givebackPct: 'ust-checklist-givebackPct',
} as const;

function useRiskGovernor(input: RiskGovernorInput): RiskGovernorState {
  const [armed] = useStickyState<boolean>(LS.armed, true);
  ...
  return useMemo(() => {
    ...
    return { profit, thresholdAmount, profitOnlyActive, remainingProfit, effectiveRisk, locked, lockReason, statusChip, maxSessionLoss, maxDailyLoss };
  }, [
    input.startBalance, input.equity, input.sessionLossSoFar, input.todayLossSoFar, input.riskAmountRequested,
    armed, thresholdPct, doNotRiskCapital, sessionMaxLossMode, sessionMaxLossValue, dailyMaxLossPct, givebackPct
  ]);
}
...
// (removed) inline useRiskGovernor below
