...
const STRATEGIES = ["Ultimate M1 Trend setup", "Ultimate M1 Range setup", "Withdrawals", "Deposits"] as const;
...
function PageInner() {
  ...
  const [planningCosts, setPlanningCosts] = useLocalStorage<PlanningCost[]>("ust-planning-costs", [
    { id: "rent", name: "Rent / Bond", amount: 0 },
    { id: "food", name: "Food", amount: 0 },
    { id: "transport", name: "Transport", amount: 0 },
    { id: "family", name: "Family / School", amount: 0 },
  ]);

  /* Derived */
  const tradeEntries = useMemo(() => trades.filter((t) => (t.kind || "trade") === "trade"), [trades]);
  const depositsTotal = useMemo(() => trades.filter((t) => t.kind === "deposit").reduce((a, t) => a + Math.abs(t.amount ?? t.pnl ?? 0), 0), [trades]);
  const withdrawalsTotal = useMemo(() => trades.filter((t) => t.kind === "withdrawal").reduce((a, t) => a + Math.abs(t.amount ?? t.pnl ?? 0), 0), [trades]);
  const totalPnlAllTime = useMemo(
    () => tradeEntries.reduce((acc, t) => acc + (t.pnl || 0), 0),
    [tradeEntries]
  );
  const progressEquity = useMemo(() => startBalance + totalPnlAllTime, [startBalance, totalPnlAllTime]);
  const equity = useMemo(() => startBalance + depositsTotal - withdrawalsTotal + totalPnlAllTime, [startBalance, depositsTotal, withdrawalsTotal, totalPnlAllTime]);
  ...
  useEffect(() => {
    const totalNeed = planningCosts.reduce((a, c) => a + (Number(c.amount) || 0), 0);
    if (totalNeed > 0) {
      const weekly = totalNeed / 4.33;
      const daily = totalNeed / 22;
      setMonthlyTarget(Number(totalNeed.toFixed(2)));
      setWeeklyTarget(Number(weekly.toFixed(2)));
      setDailyTarget(Number(daily.toFixed(2)));
    }
  }, [planningCosts, setMonthlyTarget, setWeeklyTarget, setDailyTarget]);

  const sessionTrades = useMemo(
    () => tradeEntries.filter((t) => !sessionId || (t.ts || 0) >= Number(sessionId)),
    [tradeEntries, sessionId]
  );
  ...
  const weeklyProgress = useMemo(
    () => tradeEntries.filter((t) => t.ts && isSameISOWeek(new Date(t.ts), today)).reduce((a, t) => a + (t.pnl || 0), 0),
    [tradeEntries, today]
  );
  const monthlyProgress = useMemo(
    () => tradeEntries.filter((t) => t.ts && isSameMonth(new Date(t.ts), today)).reduce((a, t) => a + (t.pnl || 0), 0),
    [tradeEntries, today]
  );
  ...
  const badgeTradesCount = tradeEntries.length;
  const [badge, setBadge] = useState<{ name: string; imagePath: string } | null>(null);
  useEffect(() => {
    const s = badgeTradesCount;
    if (s >= 200) setBadge({ name: "Legendary • 200 Trades Proven", imagePath: "/badges/legendary.png" });
    else if (s >= 150) setBadge({ name: "Elite • 150 Trades Verified", imagePath: "/badges/elite.png" });
    else if (s >= 100) setBadge({ name: "Diamond • 100 Trades Mastered", imagePath: "/badges/diamond.png" });
    else if (s >= 75) setBadge({ name: "Platinum • 75 Trades Disciplined", imagePath: "/badges/platinum.png" });
    else if (s >= 50) setBadge({ name: "Gold • 50 Trades Built", imagePath: "/badges/gold.png" });
    else if (s >= 25) setBadge({ name: "Silver • 25 Trades Logged", imagePath: "/badges/silver.png" });
    else setBadge(null);
  }, [badgeTradesCount]);

  const equitySeries = useMemo(() => {
    const sorted = [...tradeEntries].sort((a, b) => (a.ts || 0) - (b.ts || 0));
    ...
    return pts;
  }, [tradeEntries, startBalance]);

  /* Trades helpers */
  function addTrade(t: Omit<TradeRow, "id" | "ts">) {
    if (locked && lockOnHit) return;
    const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const isWithdrawal = t.kind === "withdrawal" || t.symbol === "Withdrawals" || t.notes === "Withdrawals";
    const isDeposit = t.kind === "deposit" || t.symbol === "Deposits" || t.notes === "Deposits";
    const kind: TradeRow["kind"] = isWithdrawal ? "withdrawal" : isDeposit ? "deposit" : "trade";
    const cashAmount = Math.abs(Number(t.amount ?? t.pnl ?? 0));
    const row: TradeRow = {
      id,
      ts: Date.now(),
      ...t,
      kind,
      amount: kind === "trade" ? undefined : cashAmount,
      pnl: kind === "trade" ? Number(t.pnl || 0) : 0,
    };
      ...
      <Tabs defaultValue="dashboard">
        ...
        <TabsContent value="dashboard" className="space-y-4">
          <div className="grid md:grid-cols-4 gap-4">
            ...
            />
            <DashCard title="Trade Badge" value={`${badgeTradesCount} trades`} hint={badge ? badge.name : "Starter"} />
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <DashCard title="Growth" value={`${fmt(allTimeGrowthPct)}%`} hint="Trade PnL ÷ starting capital" />
            <DashCard title="Withdrawn" value={`-${currency(withdrawalsTotal)}`} hint={`Deposited: ${currency(depositsTotal)}`} />
          </div>
            ...
            <Card>
              <CardContent className="p-5 space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  <div>
                    <Label>Daily Target (USD)</Label>
                    <Input type="number" value={dailyTarget} onChange={(e) => setDailyTarget(Number(e.target.value) || 0)} />
                  </div>
                  <div>
                ...
                <GoalProgress label="Daily Progress" progress={Number(todayPnl.toFixed(2))} target={dailyTarget || 0} />
          ...
          <BadgeShowcase badge={badge} tradesCount={badgeTradesCount} />
        ...
        <TabsContent value="analytics">
          <AnalyticsPanel trades={tradeEntries} />
        </TabsContent>
          ...
          <Card>
            <CardContent className="p-4 space-y-3">
              ...
              <div className="space-y-3">
                {MARKET_OPTIONS
                  .filter((m) => m !== "Withdrawals" && m !== "Deposits")
                  .map((mkt) => (
        ...
        <TabsContent value="calendar">
          <OldCalendar trades={tradeEntries} />
        </TabsContent>
...
function BadgeShowcase({
  ...
  const tiers = [
    ...
    { key: "Gold", name: "Gold • 50 Trades Built", at: 50, img: "/badges/gold.png" },
    { key: "Platinum", name: "Platinum • 75 Trades Disciplined", at: 75, img: "/badges/platinum.png" },
    { key: "Diamond", name: "Diamond • 100 Trades Mastered", at: 100, img: "/badges/diamond.png" },
    { key: "Elite", name: "Elite • 150 Trades Verified", at: 150, img: "/badges/elite.png" },
    { key: "Legendary", name: "Legendary • 200 Trades Proven", at: 200, img: "/badges/legendary.png" },
  ];
  const current = badge ?? (tradesCount >= 25 ? { name: tiers[0].name, imagePath: tiers[0].img } : null);
  const nextTier = tiers.find((t) => tradesCount < t.at);
  const pct = nextTier ? Math.min(100, Math.round((tradesCount / nextTier.at) * 100)) : 100;
  const left = nextTier ? Math.max(0, nextTier.at - tradesCount) : 0;
        ...
        <div className="grid md:grid-cols-3 gap-5 items-center">
          ...
          <div className="md:col-span-2">
            ...
            <div className="text-sm text-slate-600 mt-1">
              Journal trades counted: <strong>{tradesCount}</strong>
            </div>
            {nextTier ? (
              <>
              ...
              <div className="text-sm text-emerald-600 mt-3">Legendary status reached. 🏆</div>
            )}
          </div>
        </div>
      </CardContent>
...
function PlanningPage({
  ...
  function addCost() {
    setCosts([...costs, { id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, name: "New Cost", amount: 0 }]);
  }
  function removeCost(id: string) {
    setCosts(costs.length > 1 ? costs.filter((c) => c.id !== id) : costs);
  }
    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="p-5 space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-xl font-semibold">UST Planning Page</h3>
              <p className="text-sm text-slate-600">
                Add realistic monthly costs. UST will convert them into daily, weekly, monthly and yearly targets.
              </p>
            </div>
          ...
          <div className="space-y-3">
            {costs.map((c, idx) => (
              <div key={c.id} className="grid md:grid-cols-12 gap-3 items-end rounded-lg border p-3">
                <div className="md:col-span-1 text-xs text-slate-500">#{idx + 1}</div>
                <div className="md:col-span-7">
                  <Label>Monthly Cost Name</Label>
                  <Input value={c.name} onChange={(e) => updateCost(c.id, { name: e.target.value })} />
                </div>
                <div className="md:col-span-3">
                  <Label>Amount (USD)</Label>
                  <Input type="number" step="0.01" value={c.amount} onChange={(e) => updateCost(c.id, { amount: Number(e.target.value) || 0 })} />
                </div>
                <div className="md:col-span-1">
                  <Button variant="destructive" onClick={() => removeCost(c.id)}>Remove</Button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <DashCard title="Daily Target" value={currency(dailyTarget)} hint="Monthly costs ÷ 22 trading days" />
        <DashCard title="Weekly Target" value={currency(weeklyTarget)} hint="Monthly costs ÷ 4.33 weeks" />
        <DashCard title="Monthly Target" value={currency(monthlyTarget)} hint="Total estimated monthly needs" />
        <DashCard title="Yearly Target" value={currency(yearlyTarget)} hint="Monthly target × 12" />
      </div>

      <Card>
        <CardContent className="p-5 text-sm text-slate-600">
          These targets now feed the Dashboard progress cards automatically. This keeps UST focused on realistic needs instead of emotional daily targets.
        </CardContent>
      </Card>
    </div>
  );
